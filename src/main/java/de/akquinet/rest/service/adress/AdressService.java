package de.akquinet.rest.service.adress;
...
public class AdressService {
	...
	static {
		...
		adressMap.put(4l, new Address(4l, Anreden.Herr, "Mischa", "Maisky",
				"mm@test.de"));
		adressMap.put(5l, new Address(5l, Anreden.Herr, "Ludwig",
				"van Beethoven", "vonBeethoven@test.de"));
    ...
    @Path("anreden")
    @Produces(MediaType.APPLICATION_JSON)
	public List<Anreden> getAnredenListe() {
		Anreden[] values = Anreden.values();
		List<Anreden> anredenListe = Arrays.asList(values);
		return anredenListe;
	}
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Address deleteAddresse(@PathParam("addressId") long addressId) {
		Address address = adressMap.get(addressId);
